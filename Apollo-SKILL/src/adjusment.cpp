...
void testFlyWheel(){
    for (int i = 0; i <= 20; ++i){
        Motor_FlyWheel1.spin(directionType::fwd, 5 * i * 127, voltageUnits::mV);
        Motor_FlyWheel2.spin(directionType::fwd, 5 * i * 127, voltageUnits::mV);
        this_thread::sleep_for(5000);
        double spd = (Motor_FlyWheel1.velocity(velocityUnits::rpm) + Motor_FlyWheel2.velocity(velocityUnits::rpm)) / 2.0;
        cout << 5 * i << " " << spd << endl;
    }
}
